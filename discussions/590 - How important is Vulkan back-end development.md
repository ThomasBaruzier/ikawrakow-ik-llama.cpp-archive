...
👤 **os360** commented on **2025-10-25** at **15:44:05**

Vulkan is the closest we have to a cross-architecture GPU API. Improving support would help a lot of people. What we don't have a good CPU-only driver (Lavapipe is less than useless), so we can't code to Vulcan everywhere. Alas, creating a driver is definitely non-trivial, and we'd need three because Linux, BSD, and Windows drivers are so different.

Would implementing MTP in DeepSeek, GLM, and Qwen-Next be more fun? There'd be just as big a payoff there.