...
👤 **saood06** commented on **2025-08-07** at **02:21:42**

Sorry about the long delay in testing more, but I started testing again and noticed that it uses the cache quantization parameters of the main model for the draft model, but it doesn't use the flash attention of the main model (and I don't see any way to set flash attention on for the draft model), which leads to it erroring out with `V cache quantization requires flash_attn` if you use V cache quantization for the main model. 

I also was able to reproduce the `GGML_ASSERT(src0->ne[2] == src1->ne[2] && src0->ne[2] == dst->ne[2]) failed` error that ChicoPinto70 saw on my 3090 (on Windows) when testing with the model fully on the CPU, and the draft model fully on my GPU. Not sure at all what the cause is right now though.

---

👤 **ikawrakow** commented on **2025-08-07** at **05:09:57**

Status?

It looks like it works for 1 user and does not work for 2.

---

👤 **saood06** commented on **2025-08-07** at **05:54:47**

>Status?
>
>It looks like it works for 1 user and does not work for 2.

Well it did work for me on my server (where it was CPU for main model and draft), but there does seem to be an issue with placing the draft model on a CUDA device. 

I don't see the issue in the code yet, and I still haven't found a config that does work for me. I know it did work for usrlocalben, but it didn't for ChicoPinto70 who tested basically the same model (that draft model is based on Qwen2.5). I ended up testing a draft model based on llama2 for a llama2 based main model, and ran into the same issue as him.

It would be nice if there was more config for the draft model. The lack of being able to turn on flash attention or even configure cache quantization differently from the main model would be really nice and it seems like it would be simple to fix. 

More challenging would be having better configuration on what device the draft model (or it's tensors) are computed is. I made an attempt at that. I actually don't think `-devd` is the best approach but it seemed like it was the easiest to attempt to implement. Some form of `-ot` for the draft model seems possible and would be the most flexible.

<details><summary>Diff for my hacky attempt at adding `-devd` (as mentioned in a comment earlier it crashes but most of this code should be salvageable)</summary>

```diff
diff --git a/common/common.cpp b/common/common.cpp
index 1801da039..b23d35b4d 100644
--- a/common/common.cpp
+++ b/common/common.cpp
@@ -353,6 +353,36 @@ std::vector<std::pair<T1,T2>> string_split_pairs(const std::string & str, char d
 }
 }

+bool parse_device_list(const std::string& value, ggml_backend_buffer_type_t& device) {
+      if (value.empty()) {
+          fprintf(stderr, "error: no device specified in argument '%s'\n", value.c_str());
+          return false;
+      }
+
...
+          }
+      }
+      if (buft_list.find(value) == buft_list.end()) {
...
@@ -1080,6 +1110,14 @@ bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_pa
         }
         return true;
     }
+    if (arg == "--device-draft" || arg == "-devd") {
+       CHECK_ARG
+       if (!parse_device_list( argv[i], params.draft_device)) {
...
index 99048cd2a..5e843c759 100644
--- a/common/common.h
+++ b/common/common.h
@@ -145,6 +145,9 @@ struct gpt_params {
     std::vector<llama_model_tensor_buft_override> tensor_buft_overrides;
     std::vector<std::pair<int,int>> offload_policy;

+    ggml_backend_buffer_type_t draft_device;
+    bool speculative_mode = false; //useful to know if call is in speculative code
+
     bool lora_init_without_apply = false; // only load lora to memory, but do not apply it to ctx (user can manually apply lora later using llama_lora_adapter_apply)
     std::vector<llama_lora_adapter_info> lora_adapters; // lora adapter path with user defined scale

diff --git a/examples/server/server.cpp b/examples/server/server.cpp
index d0ee1731e..4905c9abe 100644
--- a/examples/server/server.cpp
+++ b/examples/server/server.cpp
@@ -909,6 +909,7 @@ struct server_context {
             LOG_INFO("loading draft model", {{"model", params.model_draft}});

             gpt_params params_dft;
+            params_dft.tensor_buft_overrides.insert(params.tensor_buft_overrides.begin(), {llama_model_tensor_buft_override{".*", params.draft_device}, llama_model_tensor_buft_override{nullptr,nullptr}}); //overide to use devd
...
index 1bc1bdafc..56a0858b6 100644
--- a/include/llama.h
+++ b/include/llama.h
...
+        ggml_backend_buffer_type_t * device; //used for the setting draft model to a device

         // main_gpu interpretation depends on split_mode:
         // LLAMA_SPLIT_NONE: the GPU that is used for the entire model
diff --git a/src/llama.cpp b/src/llama.cpp
index 27647c9d2..02bdb888f 100644
--- a/src/llama.cpp
+++ b/src/llama.cpp
@@ -20731,6 +20731,7 @@ struct llama_model_params llama_model_default_params() {
         /*.n_gpu_layers                =*/ 0,
         /*.mla                         =*/ 0,
         /*.split_mode                  =*/ LLAMA_SPLIT_MODE_LAYER,
+        /*.draft_device                =*/ nullptr,
         /*.main_gpu                    =*/ 0,
         /*.tensor_split                =*/ nullptr,
         /*.rpc_servers                 =*/ nullptr,
@@ -20920,6 +20921,7 @@ struct llama_model * llama_load_model_from_file(
         }
...
👤 **g2mt** commented on **2025-08-07** at **07:22:35**

> Sorry about the long delay in testing more, but I started testing again and noticed that it uses the cache quantization parameters of the main model for the draft model, but it doesn't use the flash attention of the main model (and I don't see any way to set flash attention on for the draft model), which leads to it erroring out with `V cache quantization requires flash_attn` if you use V cache quantization for the main model.

@saood06 I added the -ctkd, -ctvd args. Could you try them with f16?

---

👤 **saood06** commented on **2025-08-07** at **08:58:32**

>I added the -ctkd, -ctvd args. Could you try them with f16?

It worked. (I tested with bf16 instead of f16). 

Interestingly enough cache quantization on the draft model seems to be the cause of the CUDA error, as I tested this on my 3090 machine and it avoided the `GGML_ASSERT(src0->ne[2] == src1->ne[2] && src0->ne[2] == dst->ne[2]) failed` and actually loaded up completely (haven't run and actual inference yet though), so it seems that applying cache quantization to the draft model seems to cause some problems (maybe just on CUDA?). Looking at ChicoPinto70's command he used `-ctk q8_0` and I was also use K and V quantization when testing on my 3090 machine, but I wasn't when I tested on my non GPU server, so I'm not sure if that breaks on non CUDA. I'm heading off for the night, won't be able to test more and find the answer.

If you could try on your machine (I know you said you didn't have a GPU), to see if applying cache quantization works that would let us know if it is an issue with cache quantization on the draft model in general or if it just breaks with certain backends like CUDA.

---

👤 **ikawrakow** commented on **2025-08-07** at **09:04:41**

Can someone give me simple instructions how I can test this with a model+draft that works on my setup (16 GB VRAM, 128 GB RAM, or just 128 GB RAM)? Thanks.

---

👤 **saood06** commented on **2025-08-07** at **09:12:23**

> Can someone give me simple instructions how I can test this with a model+draft that works on my setup (16 GB VRAM, 128 GB RAM, or just 128 GB RAM)? Thanks.

Since you already have llama 2, a suitable draft model could be https://huggingface.co/TinyLlama/TinyLlama-1.1B-Chat-v1.0.

Make or download a GGUF for that, and the very basic llama-server command would look something like 

`.\llama-server -m [MAIN MODEL] -md [DRAFT model] -ngld [99 to place draft model on the GPU, 0 to place draft model on CPU ] -ctkd [f16/bf16/...] -ctvd [f16/bf16/...]`

where ctkd and ctvd set ctk and ctv for the draft model. You can also add flags like `-fa` or set threads or any other flag you normally use (e.g `--numa distribute -t 48 -mla 3 -fa -fmoe -c 64000 --host 0.0.0.0 -b 4096 -ub 4096 -amb 1024` for me on Deepseek on my server) to run models on your machine.

In theory you can use the same model at a lower quantization to draft from if you don't want to download a different model or want to have a much higher hit rate but generally the advice I have seen is to aim for a size ratio of around/at least (depending on who you ask) 10x between draft and main model for good performance gains.

Edit: The main constraints for a draft model are matching vocabulary (there is some leeway), and similar outputs (no point in drafting if the main model will reject every draft token if they do not give even remotely similar outputs). There are other approaches that loosen both these constraints but that is not what is in this PR (and those approaches are in general far less tested and often trade performance to get flexibility on those things).

---

👤 **ikawrakow** started a conversation on `examples/server/server.cpp` on **2025-08-07** at **13:49:06**

Add here
```
params_dft.flash_attn = params.flash_attn;
```
Otherwise the draft model does not use flash attention, which results in not being able to use quantized V cache in the main model. One could also add a separate flash attention parameter for the draft model, but I think it is OK to just use the FA setting for the main model.

> 👤 **saood06** replied on **2025-08-08** at **04:35:29**
> 
> >One could also add a separate flash attention parameter for the draft model, but I think it is OK to just use the FA setting for the main model.
> 
> I don't see why not just make it a flag where if not passed it is shadowed similar to the cache quantization.

---

👤 **ikawrakow** commented on **2025-08-07** at **13:58:29**

So, with the change in the comment above, I'm able to run with quantized KV cache CPU-only. Using
* LlaMA-2-7B as the main model (downloaded the `Q4_K_S` variant from TheBloke)
* [TinyLlaMA](https://huggingface.co/TinyLlama/TinyLlama-1.1B-Chat-v1.0.) quantized as `Q8_0` as the draft model
* `Q8_0` KV cache
* Prompt: "Write an essay about the importance of team collaboration"

TG goes up to 18.5 t/s from 13.5 t/s on my Ryzen-7950X CPU (it generates about 600 tokens).

---

👤 **ikawrakow** commented on **2025-08-07** at **14:00:21**

CUDA works too for the above setup, also with quantized KV cache. Don't see any performance gains, so I guess the setup is not useful to test CUDA speculative performance.

---

👤 **ikawrakow** approved this pull request ✅ on **2025-08-07** at **14:16:45**
...
👤 **usrlocalben** commented on **2025-08-07** at **15:12:00**

I grabbed the latest. It still (mostly) works for me.

_However_, I have observed a problem. Unfortunately it seems to be highly prompt dependent and I haven't been able to work out what the exact conditions are to produce badness.
...
edit: for reference, my config is [up above in this thread](https://github.com/ikawrakow/ik_llama.cpp/pull/645#issuecomment-3122249859) triggered with ubergarm's IQ3 and IQ4 quants although I suspect the quant itself has nothing to do with it.

---

👤 **g2mt** commented on **2025-08-07** at **15:24:11**
...
👤 **ChicoPinto70** commented on **2025-08-07** at **16:45:24**
...
👤 **saood06** commented on **2025-08-08** at **04:39:51**
...
👤 **saood06** commented on **2025-08-08** at **05:01:38**
...
👤 **ikawrakow** commented on **2025-08-08** at **11:42:44**
...
👤 **ChicoPinto70** commented on **2025-08-08** at **13:09:18**
...
👤 **ikawrakow** commented on **2025-08-08** at **13:16:49**
...
👤 **ChicoPinto70** commented on **2025-08-08** at **13:17:54**
...
👤 **ChicoPinto70** commented on **2025-08-08** at **14:12:01**
...
👤 **ChicoPinto70** commented on **2025-08-08** at **15:47:55**
...
👤 **usrlocalben** commented on **2025-08-08** at **17:34:21**
...
[short discussion](https://huggingface.co/jukofyork/Kimi-K2-Instruct-DRAFT-0.6B-v2.0-GGUF/discussions/1#687c13ef7beff569c0e50b00)
[lengthy discussion](https://github.com/ggml-org/llama.cpp/discussions/10466)

---

👤 **ChicoPinto70** commented on **2025-08-08** at **18:08:03**
...
👤 **ikawrakow** commented on **2025-08-15** at **06:22:25**

Status?

---

👤 **saood06** commented on **2025-08-15** at **07:52:39**
...
It might be useful to run in verbose (and maybe even with reverting this https://github.com/ikawrakow/ik_llama.cpp/pull/645/commits/946fa65c573e1622f599b4a56c1dc28d5a89a4ad, and see if that can be reproduced, because I do feel like it happens because the draft model is generating a LOT of garbage tokens which could tank performance (as rejecting that many tokens means doing long batches and only using the first token).

---

👤 **ChicoPinto70** commented on **2025-08-15** at **23:30:12**
...
Than, a coupe days ago, I saw ikawrakow was working in a PR ([#689](https://github.com/ikawrakow/ik_llama.cpp/issues/689)) to improve TG in MoE models. I tried it but, as the models I use are all large MoE, I didn't see any great improvement. 

But I wondered: What If the PR helps draft models? So, a couple days ago, I merged this PR with the [#689](https://github.com/ikawrakow/ik_llama.cpp/issues/689) to check it and, unfortunatelly, I didn't see any improvement..... But, now, I can test speculative decoding with the new GLM 4.5 (I had just tested before with Deepseek because Kimi K2 almost don't fit in my machine and the draft of Qwen 3 don't work with ik_llama).
...
👤 **g2mt** commented on **2025-08-16** at **00:12:32**

Once this is merged, I'll probably start working on getting universal assisted decoding in (https://github.com/ggml-org/llama.cpp/pull/12635). It should allow loading draft models with vocabulary data different from the main model.

---

👤 **whatever1983** commented on **2025-08-16** at **07:28:25**
...
👤 **ChicoPinto70** commented on **2025-08-20** at **00:55:56**
