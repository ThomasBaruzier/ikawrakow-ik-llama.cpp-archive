...
👤 **ubergarm** commented on **2025-08-08** at **18:41:10**

Well one more vote suggesting this PR improves perplexity for IQ3_KS.

Giving this PR another shot with two versions of GLM-4.5-Air-IQ3_KS 49.072 GiB (3.816 BPW). Using the same recipe with the PR and main which uses 90x `IQ3_KS` for all the routed expert `ffn_(gate|up)_exps` tensors. None of the other quantization types used in this recipe are involved with this PR.

| Version | Final Perplexity |
| --- | --- |
| main | 4.8086 +/- 0.02985 |
| PR624 | 4.7975 +/- 0.02972 |

<details>

<summary>👈 GLM-4.5-AIr-IQ3_KS Recipe </summary>
...
<img width="2051" height="1400" alt="ppl-GLM-4 5-Air" src="https://github.com/user-attachments/assets/bd73cbe9-d38a-4d6f-a79e-2ddc00be7f6d" />

---

👤 **ubergarm** commented on **2025-08-22** at **17:42:22**

One more vote for this PR624 over main.

On ubergarm/DeepSeek-V3.1-IQ2_KL 231.206 GiB (2.960 BPW) using the same recipe with `IQ3_KS` for all the routed expert `ffn_down_exps` tensors. None of the other quantization types used in this recipe are involved with this PR.

| Version | Final Perplexity |
| --- | --- |
| main | 3.6388 +/- 0.02172 |
| PR624 | 3.6312 +/- 0.02161 |
...
<img width="2056" height="1400" alt="ppl-DeepSeek-V3 1" src="https://github.com/user-attachments/assets/2a24e194-c90c-40d0-a3a6-8611052dc4e3" />

It is kinda slow to cook each quant twice to check all of these, but in general this PR seems to outperform main so I've been releasing some quants using this PR at this point.

---

👤 **ubergarm** commented on **2025-08-23** at **05:03:30**

PR624 gives better perplexity again over main. I couldn't sleep after releasing the "good" one, so I had to check just to confirm it was indeed better haha...

Similar quant as above, [ubergarm/DeepSeek-V3.1-IQ2_KS](https://huggingface.co/ubergarm/DeepSeek-V3.1-GGUF#iq2_ks-193144-gib-2472-bpw) 193.144 GiB (2.472 BPW). Exact recipe in the link, but basically it uses  `IQ2_KS` for all the routed expert `ffn_(gate|up)_exps` tensors. None of the other quantization types used in this recipe are involved with this PR.

| Version | Final Perplexity |
| --- | --- |
| main | 3.9624 +/- 0.02438 |
| PR624 |  3.9583 +/- 0.02433|

<img width="2069" height="1400" alt="ppl-DeepSeek-V3 1-PR624" src="https://github.com/user-attachments/assets/afa61bf8-7a47-4b74-9467-9e9f210c8dbd" />

---

👤 **sayap** commented on **2025-09-22** at **15:12:52**

I am testing a couple of GLM-4.5 quants with:
* llama_model_loader: - type iq3_ks:  180 tensors (50% of ffn_(up|gate)_exps, 100% of ffn_down_exps)
* llama_model_loader: - type iq2_kl:   90 tensors (50% of ffn_(up|gate)_exps)

quant made from main:
```
Final estimate: PPL = 3.6206 +/- 0.02109
```

quant made from pr624 rebased on main:
```
Final estimate: PPL = 3.6275 +/- 0.02101
```

This one uses the imatrix from https://huggingface.co/mradermacher/GLM-4.5-i1-GGUF/blob/main/GLM-4.5.imatrix.gguf