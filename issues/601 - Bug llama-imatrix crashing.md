...
👤 **Lissanro** commented on **2025-07-28** at **11:11:36**

@ubergarm 
> Both myself and anikifoss have converted Kimi-K2 using this repo's convert_hf_to_gguf.py script

Maybe I did something wrong but it turned out that  K2 specific tokenizer and self-attention related metadata not being added by the evshiron's convert_hf_to_gguf.py script. I shared details here https://github.com/ikawrakow/ik_llama.cpp/issues/658 . So I am considering trying the other way you described:

> I've written about all three methods to get fp8 safetensors into bf16 GGUFs here: https://github.com/ggml-org/llama.cpp/issues/14762#issuecomment-3098571703
...
👤 **onestardao** commented on **2025-07-28** at **13:02:19**

Thanks for everyone's great work and deep dives here — I've read through the entire thread and wanted to contribute a broader systems view that might help resolve this and similar crashes in a more reproducible and explainable way.

---

## Root of the problem: Metadata misalignment and toolchain expectation mismatch

From my perspective, the crash seems tightly linked to a silent divergence between:

1. **GGUF metadata completeness** — especially regarding tokenizer specs and attention tensor details (`attn_k_b`, `attn_v_b`, etc).
2. **Toolchain expectation** — `llama-imatrix` appears to expect data that wasn't properly written into GGUF (due to either upstream convert script or tokenizer gaps).
3. **Quantization alignment** — The behavior of `--alias=3` vs `--alias=1` changes what gets included, but this is not always obvious from the CLI perspective, making errors hard to debug unless you're tracking tensor-level metadata generation.

---

## My suggested approach:

Instead of patching each broken case reactively, we can approach this like a "reasoning engine" mismatch problem. Here's what might help:

### 1. Build a **GGUF sanity checker** script:
This would parse the output GGUF and check if required tensors (`attn_k_b`, `attn_v_b`, routing metadata) are present and well-quantized. Think of it like a "pre-flight check" before passing it into `llama-imatrix`.

> If you want, I can help draft a minimal version of this — I’ve built something similar in another context.

---

### 2. Use **explainable failure paths** in `llama-imatrix`
Right now, crashing with `fatal error` leaves people digging through source. Can we instead print:

```

\[llama-imatrix] Missing attn\_k\_b / attn\_v\_b tensors in GGUF metadata.
Please ensure you use --alias=1 during conversion and that all routing metadata is properly serialized.

```

Even a short suggestion like that would save hours of collective effort.

---

### 3. Consider pre-bundled **reference GGUFs** (for test-only purposes)
This would allow users to test toolchains without model conversion overhead — a known-good GGUF for each major quant config (Q4_K_M, etc), e.g. Kimi-K2 or DeepSeek variants.

---

Let me know if this would be useful to the maintainers — happy to PR a GGUF checker, or provide a test harness for sanity validation. I’m working on a reasoning framework (called WFGY) that precisely tackles these “semantic misalignment bugs” and am very interested in improving robustness here.

If anyone wants to sync outside GitHub, feel free to ping me on Telegram or GitHub discussions (my DMs are open).

---

👤 **onestardao** commented on **2025-07-28** at **13:02:48**

**🛠️ Want to reproduce this successfully? Here's a working GGUF conversion command (for DeepSeek V3):**
...
* The tokenizer path points to the correct `tokenizer.model` file (not left blank)
* You’re on the latest convert.py script (some fields like attention tensor metadata were only added recently)
* After conversion, validate with: `gguf-inspect output.gguf | grep attn_`